...
def test_quick_sample_lineargam_y():
    ...
    gam = LinearGAM(s(0, spline_order=2, n_splines=5, lam=0.1), fit_intercept=False)
    gam.fit(X, y)

    y_pred = gam.predict(X)
    y_samples = predict.quick_sample(
        gam=gam, sample_at_X=X, quantity="y", n_draws=200, random_seed=0
    )
...
def test_quick_sample_logisticgam_mu():
    ...
    gam = LogisticGAM(s(0, spline_order=2, n_splines=5, lam=0.001), fit_intercept=False)
    gam.fit(X, y)

    y_samples = predict.quick_sample(
        gam=gam, sample_at_X=X, quantity="mu", n_draws=100, random_seed=0
    )
